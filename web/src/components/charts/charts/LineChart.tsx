    ...
    <div className="line-chart w-full h-full relative">
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {seriesGroups.map((seriesValue, seriesIndex) => {
          ...
          // 简单计算坐标 (这里只是模拟)
          const xValues = [
            ...new Set(seriesData.map((item) => item[dimensionField])),
          ];
          const yValues = seriesData.map(
            (item) => parseFloat(item[measureField]) || 0
          );
          ...
          const points = seriesData.map((item, i) => {
            ...
            const y =
              height -
              40 -
              ((parseFloat(item[measureField]) || 0) / maxY) * (height - 60);
            ...
            <g key={seriesIndex}>
              ...
              {points.map((point, i) => (
                <circle
                  ...
                  strokeWidth="2"
                  onClick={(e) =>
                    handleDataPointClick?.(point.data, e as React.MouseEvent)
                  }
                  onMouseEnter={(e) =>
        ...
        {data.length > 0 &&
          (() => {
            const xValues = [
              ...new Set(data.map((item) => item[dimensionField])),
            ];
