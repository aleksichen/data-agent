...
const BarChart: React.FC<BarChartProps> = ({
  ...
  if (isStacked) {
    ...
    categories.forEach(category => {
      const categoryData = data.filter(item => item[dimensionField] === category);
      const sum = categoryData.reduce((acc, item) => acc + (parseFloat(item[measureField]) || 0), 0);
      maxValue = Math.max(maxValue, sum);
    });
  } else {
    // 对于分组图，找出最大值
    maxValue = Math.max(...data.map(item => parseFloat(item[measureField]) || 0));
  }
    ...
    <div className="bar-chart w-full h-full relative">
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {categories.map((category, categoryIndex) => {
          const categoryX = 40 + categoryIndex * groupWidth + groupWidth * barPadding / 2;
          ...
          return (
            <g key={categoryIndex}>
              {seriesValues.map((seriesValue, seriesIndex) => {
                // 筛选当前类别和系列的数据
                let barData = data.filter(item => item[dimensionField] === category);
                if (seriesField && seriesValue !== null) {
                  barData = barData.filter(item => item[seriesField] === seriesValue);
                }
                ...
                return (
                  <g key={seriesIndex}>
                    ...
                    <rect
                      ...
                      height={barHeight}
                      fill={color}
                      onClick={(e) => handleDataPointClick?.(barData[0], e as React.MouseEvent)}
                      onMouseEnter={(e) => handleHover?.(barData[0], e as React.MouseEvent)}
            ...
            {seriesValues.map((value, i) => {
              ...
              return (
                <g key={i} transform={`translate(${width - 100}, ${20 + i * 20})`}>
                  ...
                  <text x="15" y="9" fontSize="10" fill="#666">{String(value)}</text>
                </g>
              );
            })}
          </g>
