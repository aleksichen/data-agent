  ...
  return (
    <div className="area-chart w-full h-full relative">
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        {/* 坐标轴 */}
        <line x1={plotLeft} y1={plotBottom} x2={plotRight} y2={plotBottom} stroke="#ccc" strokeWidth="1" />
        <line x1={plotLeft} y1={plotTop} x2={plotLeft} y2={plotBottom} stroke="#ccc" strokeWidth="1" />
        ...
        {isStacked ? (
          ...
          (() => {
            ...
            xCategories.forEach(cat => { stackedValues[String(cat)] = 0; });
            ...
            return seriesGroups.map((group, groupIndex) => {
              ...
              // 构建区域路径
              const areaPoints: { x: number, y: number, value: number, category: any, data: DataPoint }[] = [];
              ...
              xCategories.forEach((category, catIndex) => {
                ...
                const value = dataPoint ? parseFloat(dataPoint[measureField]) || 0 : 0;
                
                // X坐标 (固定)
                const x = plotLeft + (catIndex / (xCategories.length - 1 || 1)) * plotWidth;
                
                // 当前值的底部位置 (基于之前的累积值)
                const baseY = plotBottom - (stackedValues[String(category)] / maxY) * plotHeight;
                
                // 添加当前值到累积值
                stackedValues[String(category)] += value;
                
                // 当前值的顶部位置
                const y = plotBottom - (stackedValues[String(category)] / maxY) * plotHeight;
              ...
              if (areaPoints.length > 0) {
                ...
                // 添加底部线连接到第一个数据点的底部
                const baseY = plotBottom - (stackedValues[String(areaPoints[0].category)] / maxY - areaPoints[0].value / maxY) * plotHeight;
                ...
                // 添加回到底部的线
                areaPath += `L ${areaPoints[areaPoints.length - 1].x} ${plotBottom} `;
                ...
                <g key={groupIndex}>
                  ...
                  {areaPoints.map((point, i) => (
                    <circle
                      ...
                      stroke={color}
                      strokeWidth="2"
                      onClick={(e) => handleDataPointClick?.(point.data, e as React.MouseEvent)}
                      onMouseEnter={(e) => handleHover?.(point.data, e as React.MouseEvent)}
                  ...
                  {/* 图例 */}
                  {seriesField && group && (
                    <g transform={`translate(${plotRight - 100}, ${plotTop + groupIndex * 20})`}>
                      <rect width="10" height="10" fill={color} fillOpacity={fillOpacity} />
                      <text x="15" y="9" fontSize="10" fill="#666">{String(group)}</text>
                    </g>
                  )}
                </g>
              );
          ...
          seriesGroups.map((group, groupIndex) => {
            ...
            seriesData = [...seriesData].sort((a, b) => {
              return xCategories.indexOf(a[dimensionField]) - xCategories.indexOf(b[dimensionField]);
            });
            ...
            const points = seriesData.map((item, i) => {
              ...
              const x = plotLeft + (xIndex / (xCategories.length - 1 || 1)) * plotWidth;
              const y = plotBottom - ((parseFloat(item[measureField]) || 0) / maxY) * plotHeight;
