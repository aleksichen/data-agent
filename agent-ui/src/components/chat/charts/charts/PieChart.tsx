...
const PieChart: React.FC<PieChartProps> = ({
  ...
  const total = data.reduce(
    (sum, item) => sum + (parseFloat(item[measureField]) || 0),
    0
  )
  ...
  return (
    <div className="pie-chart relative h-full w-full">
        ...
        {sectors.map((sector, i) => (
          <g key={i}>
            <path
              ...
              strokeWidth="1"
              onClick={(e) =>
                handleDataPointClick?.(sector.item, e as React.MouseEvent)
              }
              onMouseEnter={(e) =>
                handleHover?.(sector.item, e as React.MouseEvent)
              }
